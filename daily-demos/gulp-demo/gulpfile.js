/*!
 * gulp
 * $ npm install -D gulp-ruby-sass gulp-cached gulp-uglify gulp-rename gulp-concat gulp-notify gulp-filter gulp-jshint gulp-rev-append gulp-cssnano gulp-imagemin browser-sync gulp-file-include gulp-autoprefixer del –save-dev
 */
...
gulp.task("styleReload", gulp.series("sass", "css"), function () {
  return gulp
    .src(["dist/css/**/*.css"])
    .pipe(cached("style"))
...
// build，关连执行全部编译任务
gulp.task("build", gulp.series("sass", "css", "script", "image"), function () {
  gulp.start("html");
});
